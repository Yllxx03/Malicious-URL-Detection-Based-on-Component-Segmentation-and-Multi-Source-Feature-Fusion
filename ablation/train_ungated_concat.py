...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ablation: remove the gate and use ungated concatenation fusion."
    )
    add_data_arguments(parser, default_output_dir=PROJECT_ROOT / "runs" / "ablation_ungated_concat")
    ...
    parser.set_defaults(
        ...
        experiment_name="ungated_concat",
        reference_experiment="main_hybrid_delta_attention",
        experiment_description="移除门控融合，用简单拼接替代，用于验证门控融合本身是否提供增益。",
        ablation_factors=(
            "仅修改 fusion_mode：gated_sum -> ungated_concat。",
