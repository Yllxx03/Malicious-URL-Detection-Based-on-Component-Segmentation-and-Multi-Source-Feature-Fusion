...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ablation: replace attention pooling in the optimized model with mean pooling."
    )
    add_data_arguments(parser, default_output_dir=PROJECT_ROOT / "runs" / "ablation_mean_pooling")
    ...
    parser.set_defaults(
        ...
        experiment_description="Keep the optimized encoder and conditional fusion, but replace attention pooling with mean pooling.",
        ablation_factors=(
            "sequence_pooling: attention -> mean",
            "keeps sequence_encoder=hybrid_delta_attention",
            "keeps fusion_mode=bilinear_gated",
            "tests whether sparse malicious payload signals are diluted by mean pooling",
        ),
